...
SELECT * FROM voice_states WHERE user_id = $1 LIMIT 1;
...
SET
    ...
    is_deafened = COALESCE(
        sqlc.narg ('is_deafened'),
        is_deafened
    ),
    self_mute = COALESCE(
        sqlc.narg ('self_mute'),
    ...
    self_video = COALESCE(
        sqlc.narg ('self_video'),
        self_video
    ),
    self_stream = COALESCE(
        sqlc.narg ('self_stream'),
        self_stream
    ),
...
-- name: DeleteUserVoiceStates :one
